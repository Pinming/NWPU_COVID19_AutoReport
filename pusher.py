...
当前脚本版本：`{index.__version__[0]}.{index.__version__[1]}.{index.__version__[2]}`
'''
        data_for_push = {
            "title": title,
            "desp": content,
...
- 本软件获取您所填写的地理信息时出现了错误。
    该种情况，请联系作者（通过 Github Issue 或邮箱：i@pm-z.tech）并附上信息填报网站「个人中心→我的打卡」页面的截图，便于定位问题！
                ''' + '\n' + f'疫情填报系统返回的错误信息：{error_info}' + '\n' + f'当前脚本版本：`{index.__version__[0]}.'
                                                                    f'{index.__version__[1]}.{index.__version__[2]}`'
        }
